...
import {listMovieCredits, listMovieDetails, saveMovieDetails} from "../features/movies/movieSlice";
...
const MovieScreen: React.FC = () => {
    ...
    const { topBilledActors } = useSelector((state: RootState) => state.persons);
    const { movieReviews } = useSelector((state: RootState) => state.reviews);
    const { recommendationData } = useSelector((state: RootState) => state.recommendations);
    const { movieVideo } = useSelector((state: RootState) => state.videos);
    ...
    useEffect(() => {
        if (id) {
            ...
            dispatch(fetchRecommendations({ movieId: id, type: 'movie' }));
            dispatch(fetchVideos({ movieId: id, type: 'movie' }));
        }
    }, [dispatch, id]);
        ...
        <div>
                ...
                <Row>
                    ...
                    <Col md={3}>
                        <nav>
                            ...
                            <div>
                                <strong>D {movie?.release_date} (BG).
                            ...
                            <div>
                                {movie?.production_companies?.map((genre: Genre) =>
            ...
            <div className="flex pt-8">
                <div className="white-column">
                    ...
                    <Row className="row-border-top2">
                        {topBilledActors.map((person: Person, index: number) =>
                            (index < 8) ?
                                <Col
                                    style={{width: '12%'}}
                ...
                <div className="grey-column">
                    ...
                    <div className="pb-2">
                        ...
                        {movie?.production_companies?.map((genre: Genre, index: number) => (
                            <span key={genre.id}>
                                    {genre.name}
                                {index < movie.genres.length - 1 && ', '}
                                </span>
            ...
            <div>
                ...
                <Row className="row-border-top circle-image">
                    ...
                    {movieReviews.map((review: Review, index: number) =>
                        (index < 1) ?
            ...
            <div>
                ...
                <Row className="row-border-top">
                    {movieVideo.map((movie: Movie, index: number) =>
                        (index < 1) ?
                            <Col
            ...
            <div>
                ...
                <Row className="row-border-top recommendations">
                    {recommendationData.map((recommendation: Recommendation, index: number) =>
                        (index < 4) ?
                            <Col

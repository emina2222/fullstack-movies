...
import {fetchCurrentSeason, fetchSeriesCast, fetchTvList, saveTvDetails} from "../features/tv/tvSlice";
...
const TVScreen = () => {
    ...
    const seriesCastStore = useSelector((state: RootState) => state.persons)
    const {people} = seriesCastStore

    const movieReviewsList = useSelector((state: RootState) => state.reviews)
    const {movieReviews} = movieReviewsList

    const recommendation = useSelector((state: RootState) => state.recommendations)
    const {recommendationData} = recommendation

    const currentSeason = useSelector((state: RootState) => state.tv)
    ...
    useEffect(() => {
        if (id) {
            ...
            dispatch(fetchSeriesCast(id))
            dispatch(fetchReviews({movieId: id, type: 'tv'}))
            dispatch(fetchVideos({movieId: id, type: 'tv'}))
            dispatch(fetchRecommendations({movieId: id, type: 'tv'}))
            dispatch(fetchCurrentSeason(id))
        }
    ...
    const tempTV = {
        ...
        productionCompaniesTV: tv.production_companies?.map((company: Company) => company.name),
        voteAverage: tv.vote_average * 10,
        voteCount: tv.vote_count,
        genres: tv.genres?.map((genre: Genre) => genre.name),
        theMovieDbId: tv.id,
        ...
        <div>
            ...
            <div style={{
                ...
                <Row>
                    <Col md={3}>
                        ...
                        <CardImg style={{height: '450px', width: '300px'}}
                                 src={`https://image.tmdb.org/t/p/w300_and_h450_bestv2${tv?.backdrop_path}`}
                                 variant='top'/>
                    ...
                    <Col md={3}>
                        <nav>
                            ...
                            <div>
                                <strong>D {tv?.release_date} (BG). {tv?.genres?.map((genre: Genre) => genre.name + ', ' || '')} {tv?.runtime} min </strong>
                            </div>

                            <ul className='style-ul'>
                                <li className='style-li'>
                                    <div style={{height: '50px', width: '50px'}}>
                                        <CircularProgressbar value={tv?.vote_average * 10}
            ...
            <div className="flex pt-8 justify-between">
                <div className="white-column">
                    <h3>Series Cast</h3>
                    <Row>
                        {people?.cast?.map((person: Person, index: number) =>
                            (index < 8) ?
                                <Col
                                    style={{width: '12%'}}
                                    key={person.id} sm={12} md={6} lg={4} xl={3}
                                >
                                    <PersonComponent person={person} size={size}/>
                                </Col>
                                : null
                        )}
                ...
                <div className="grey-column">
                    <div className="flex place-content-between pb-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30.4" height="30.4" fill="currentColor"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="30.4" height="30.4" fill="currentColor"
                             className="bi bi-infinity" viewBox="0 0 16 16">
                            <path
                    ...
                    <div className="pb-2">
                        ...
                        {tv?.production_companies?.map((genre: Genre) =>
                            (genre.name) + ', ')}
                    </div>
                    <div className="pb-2">
            ...
            <div>
                ...
                <Row className="gutter">
                    <CurrentSeason currentSeasonEpisode={currentSeasonEpisodes}/>
                </Row>
            </div>
            ...
            <div>
                ...
                <Row className="row-border-top recommendations">
                    ...
                    {recommendationData.map((recommendation, index) =>
                        (index < 4) ?
                            <Col
                                key={recommendation.id} sm={12} md={6} lg={4} xl={3}
                            >
